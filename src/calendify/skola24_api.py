...
class Skola24Api:
    def __init__(
        ...
        domain: str,
        school: str = "",
        xscope="8a22163c-8662-4535-9050-bc5e1923df48",
    ):
        self.domain = domain
        self._xscope = xscope
        ...
        self.headers = {"X-Scope": self._xscope, "Content-Type": "application/json"}
    ...
    def get_school(self, name):
        r = self._post(
            ...
            {"getTimetableViewerUnitsRequest": {"hostName": self.domain}},
        )
    ...
    def get_school_years(self):
        r = self._post(
            ...
            {"checkSchoolYearsFeatures": False, "hostName": self.domain},
        )
    ...
    def get_class_lessons(self, class_name: str, year: int = 2022, week: int = 1):
        return self.get_lessons(
            self.get_class(class_name)["groupGuid"], SelectionType.CLASS, year, week
        )

    def get_student_lessons(self, student_id: str, year: int = 2022, week: int = 1):
        return self.get_lessons(
            self.get_student_signature(student_id), SelectionType.STUDENT_ID, year, week
        )

    def get_lessons(
        self, selection: str, selection_type: SelectionType, year: int, week: int
    ):
        body = {
    ...
    def get_timeslot(self, class_name, timeslot):
        ...
        times = list(
            filter(
                lambda x: self.timestamp_re.match(x["text"])
                and x["fontsize"] != ignore_fontsize,
                data["textList"],
            )
        )

        lunch = list(
            filter(lambda x: timeslot.lower() in x["text"].lower(), data["textList"])
        )
        lunch_boxes = []
        ...
        for i in range(len(lunch)):
            start = nearest(lunch_boxes[i]["y"], times, "y")["text"]
                ...
                lunch_boxes[i]["y"] + lunch_boxes[i]["height"] + times[0]["fontsize"],
                times,
                "y",
            )["text"]
