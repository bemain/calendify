...
class GoogleCalendarApi:
    ...
    def generate_credentials(self) -> Credentials:
        ...
        if (
            not credentials
            or (credentials is Credentials and not credentials.valid)
            or (credentials is ServiceAccountCredentials and credentials.invalid)
        ):
    ...
    def get_calendar_id(self, calendar_name: str) -> str:
        ...
        if calendar_name in calendar_names:
            ...
            return list(
                filter(lambda calendar: calendar["summary"] == calendar_name, calendars)
            )[0]["id"]
        else:
            # Create calendar
